...
MQTT_MACHINE_NAME  = os.getenv('MQTT_MACHINE_NAME',"raspberry")
...
def on_connect(client, userdata, flags, rc):
    ...
    client.publish(MQTT_TOPIC_PREFIX+"/"+MQTT_MACHINE_NAME+"/status",payload="Online",qos=1,retain=True)

    # subscribe to command topic
    client.subscribe(MQTT_TOPIC_PREFIX+"/"+MQTT_MACHINE_NAME+"/command", MQTT_QOS)

def on_subscribe(client, userdata, mid, granted_qos):
    print(f"Subscribed to commands on topic \"{MQTT_TOPIC_PREFIX}/{MQTT_MACHINE_NAME}/command\" with QOS {granted_qos[0]}.")
    print("Wake-On-LAN proxy service started.")  
...
def on_message(client, userdata, message):
    ...
    if re.match(r"[0-9a-f]{2}([-:\.]?)[0-9a-f]{2}(\1[0-9a-f]{2}){4}$", message.payload.lower()):
        ...
        print(f"Message payload {message.payload} has invalid MAC address format")
...
client.will_set(MQTT_TOPIC_PREFIX+"/"+MQTT_MACHINE_NAME+"/status", payload="Offline", qos=1, retain=True)    
